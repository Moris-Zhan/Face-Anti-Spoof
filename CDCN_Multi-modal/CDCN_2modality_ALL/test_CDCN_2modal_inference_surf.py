...
from models.CDCNs import Conv2d_cd, CDCN_2modality

from Loadtemporal_valtest_3modality import Surfing_valtest, Normaliztion_valtest, ToTensor_valtest
...
def ValidFeatureMap( orign_img, sn, x, feature1, feature2, feature3, map_x, writer, step, epoch, spoof_label, val=False,TAG="RGB"):
    ...
    ## initial images 
    feature_first_frame = x[0,:,:,:].cuda()    ## the middle frame 
    ...
    ## first feature
    feature_first_frame = feature1[0,:,:,:].cuda()    ## the middle frame 
    heatmap = torch.zeros(feature_first_frame.size(1), feature_first_frame.size(2))
    for i in range(feature_first_frame.size(0)):
        heatmap += torch.pow(feature_first_frame[i,:,:],2).view(feature_first_frame.size(1),feature_first_frame.size(2))
    ...
    ## second feature
    feature_first_frame = feature2[0,:,:,:].cuda()    ## the middle frame 
    heatmap = torch.zeros(feature_first_frame.size(1), feature_first_frame.size(2))
    for i in range(feature_first_frame.size(0)):
        heatmap += torch.pow(feature_first_frame[i,:,:],2).view(feature_first_frame.size(1),feature_first_frame.size(2))
    ...
    ## third feature
    feature_first_frame = feature3[0,:,:,:].cuda()    ## the middle frame 
    heatmap = torch.zeros(feature_first_frame.size(1), feature_first_frame.size(2))
    for i in range(feature_first_frame.size(0)):
        heatmap += torch.pow(feature_first_frame[i,:,:],2).view(feature_first_frame.size(1),feature_first_frame.size(2))
    ...
    heatmap5 = heatmap5.data.cuda().numpy()
...
def train_test():
    ...
    model = CDCN_2modality( basic_conv=Conv2d_cd, theta=args.theta)
    
    # model.load_state_dict(torch.load('CDCN_2modality_P1/CDCN_2modality_P1_50.pkl'))
    # model.load_state_dict(torch.load(f'./{args.log}/{args.log}_last.pkl'))
    model.load_state_dict(torch.load(f'./{args.log}/{args.log}_best.pkl'))
    ...
    with torch.no_grad():
        ...
        val_data = Surfing_valtest(test_list, image_dir, transform=transforms.Compose([Normaliztion_valtest(), ToTensor_valtest()]))
        ...
        video_save_path = f"{args.log}/videos_surf/inference_surf.mp4"
        ...
        video_save_path = f"{args.log}/videos_surf/Live_surf.mp4"
        os.makedirs(os.path.dirname(video_save_path), exist_ok=True)
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        size = (1280, 720)
        video_fps = 25.0
        ...
        video_save_path = f"{args.log}/videos_surf/Spoof_surf.mp4"
        os.makedirs(os.path.dirname(video_save_path), exist_ok=True)
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        size = (1280, 720)
        video_fps = 25.0
        ...
        for i, sample_batched in enumerate(dataloader_val):
            ...
            text_label = f"Thres:{args.thres} Live_Prob:{(map_score*100):2.3f}% latency={latency:2.2f}s fps={fps:2.2f}"
        ...
        # map_score_val_filename = "./" + args.log+'/'+ "videos(ep{epoch})"+ '_map_score_test_50.txt'
        map_score_val_filename = f"./{args.log}/videos_surf/map_score_tes.txt"
        os.makedirs(os.path.dirname(map_score_val_filename), exist_ok=True)
        with open(map_score_val_filename, 'w') as file:
            file.writelines(map_score_list) 
            file.write("Epoch %d The APCER is: %f\n" % ((epoch + 1),val_APCER)) 
            file.write("Epoch %d The BPCER is: %f\n" % ((epoch + 1),val_BPCER)) 
            file.write("Epoch %d The ACER is: %f\n" % ((epoch + 1),val_ACER)) 
            file.write("Epoch %d The Accuracy is: %f\n" % ((epoch + 1),val_ACC)) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log', type=str, default="CDCN_2modality_ALL", help='log and save model name')
    parser.add_argument('--protocol', type=str, default="surf", help='log and save model name')
    ...
    # Dataset root      
    image_dir = '/home/leyan/DataSet/Surfing-2020-Anti-spoofing/data'  
    ...
    test_list = f"{image_dir}/surf_ref.txt"
    train_test()
    ...
    # python test_CDCN_inference_surf.py --log CDCN_3modality_P1_4@1
    # python test_CDCN_inference_surf.py --log CDCN_3modality_P1_4@2
    # python test_CDCN_inference_surf.py --log CDCN_3modality_P1_4@3