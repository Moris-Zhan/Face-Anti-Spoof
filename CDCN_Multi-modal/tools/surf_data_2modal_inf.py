...
"""Prepare demo_2modal file"""
video_save_path = "./tools/demo_2modal.mp4"
...
def recogn_spoof_mcdcn(rgb, depth, ir, bboxes, session=None):
    ...
    net_outs = session.run(None, 
            {
                input1_name: blob_rgb,
                # input2_name: blob_ir,
                input3_name: blob_depth
            },)
    # print(f"sess Duration Time:  {time.time()-st:2.2f}, s")
    map_x = net_outs[0]
    ...
    latency = time.time()-st
    fps  = (1/latency)
    return score_norm, latency, fps   

"""Real-People"""
def read_modal_img(res, 
        scrfd_file="./tools/scrfd_2.5g_shape256x256.onnx",
        mcdcn_file="./tools/mcdcn_2modal.onnx"):
    ...
    for dir0 in res:
        ...
        for rgb_path in rgb_paths:
            ...
            map_score = 0
            map_score, latency2, _ = recogn_spoof_mcdcn(rgb, depth, ir, bboxes, session = mcdcn_session)       
            latency = latency1 + latency2    
            fps = 1 / latency     
            print(f"Duration Time:  {latency:2.2f}, s")           
            text_label1 = f"scrfd latency={latency1:2.2f}s, mcdcn latency={latency2:2.2f}s, map_score:{(map_score*100):2.3f}% "
            ...
            os.makedirs("Multi-modal/tools/demo_2modal/", exist_ok=True)
            cv2.imwrite(f"Multi-modal/tools/demo_2modal/{fn}.jpg", img)
            out.write(img)
