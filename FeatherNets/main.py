...
parser.add_argument('--arch', '-a', metavar='ARCH', default='resnet18',
                    ...
                    help='models architecture: ' +
                         ' | '.join(model_names) +
                         ' (default: resnet18)')
# parser.add_argument('--config', default='cfgs/FeatherNetA-32.yaml')
# parser.add_argument('--config', default='cfgs/FeatherNetB-32.yaml')
parser.add_argument('--config', default='cfgs/mobilenetv2.yaml')
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (sample_batched) in enumerate(train_loader):
        ...
        input = sample_batched['image_depth'].cuda()
    ...
    with torch.no_grad():
        ...
        for i, (sample_batched) in enumerate(val_loader):
            with torch.no_grad():
                ...
                # compute output
                output = model(input_var) # torch.Size([32, 1024])
                loss = criterion(output, target_var)
