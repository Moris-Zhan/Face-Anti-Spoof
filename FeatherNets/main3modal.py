...
from utils.profile import count_params, count_params2
...
import roc
import models
from read_data import CASIA, Spoofing_valtest, Spoofing_train
from losses import *
from tools.benchmark import compute_speed, stat
from tools.data.Loadtemporal_BinaryMask_train_3modality import *
from tools.data.Loadtemporal_valtest_3modality import *
from models.CDCNs import Conv2d_cd, CDCN_3modality2, CDCN_2modality, CDCN_2modality_lowhigh
...
# parser.add_argument('--config', default='cfgs/fishnet150-32.yaml')
parser.add_argument('--config', default='cfgs/FeatherNet-modal-fusion.yaml')
...
parser.add_argument('--resume', default='checkpoints/FeatherNet_bs32_modal3/_170.pth.tar', type=str, metavar='PATH',
...
def main():
    ...
    # model = CDCN_2modality( basic_conv=Conv2d_cd, theta=0.7)
    # model = CDCN_2modality_lowhigh( basic_conv=Conv2d_cd, theta=0.7)
    ######################################################
    global writer
    writer = SummaryWriter(log_dir=os.path.join(f"work_dirs/{args.arch}", "tensorboard"))
    if args.summary: 
    ...
    input_size = tuple(int(x) for x in str_input_size.split('x'))        
    rndm_input = torch.autograd.Variable(torch.rand(input_size), requires_grad = False).cuda()
    ...
    train_dataset = Spoofing_train(train_list, image_dir, 
                                transform=transforms.Compose([
                                    RandomErasing(), RandomHorizontalFlip(),  
                                    ToTensor(), Cutout(), Normaliztion()]))
    val_dataset = Spoofing_valtest(val_list, image_dir, transform=transforms.Compose([
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (sample_batched) in enumerate(train_loader):
        ...
        rgb = sample_batched['image_x'].cuda()
        ir = sample_batched['image_ir'].cuda()
        depth = sample_batched['image_depth'].cuda()
        target = sample_batched['spoofing_label'].cuda()

        rgb = Variable(rgb).float().to(device)
        ir = Variable(ir).float().to(device)
        depth = Variable(depth).float().to(device)
        ...
        if i % args.print_freq == 0:
            os.makedirs(f"work_dirs/{args.arch}", exist_ok=True)
            with open(f'work_dirs/{args.arch}/{time_stp}_{args.arch}.log', 'a+') as flog:
            ...
            writer.add_scalar('Epoch/Loss_train', loss.data, epoch)
            writer.add_scalar('Epoch/Prec_1_train', top1.avg, epoch)
...
def validate(val_loader, model, criterion,epoch):
    ...
    with torch.no_grad():
        ...
        for i, (sample_batched) in enumerate(val_loader):
            with torch.no_grad():
                # depth = Variable(input).float().to(device)
                # target_var = Variable(target).long().to(device)

                rgb = sample_batched['image_x'].cuda().squeeze(1)
                ir = sample_batched['image_ir'].cuda().squeeze(1)
                depth = sample_batched['image_depth'].cuda().squeeze(1)

                # input = sample_batched['image_depth'].cuda().squeeze(1)
                target = sample_batched['live'].cuda()
                depth_dirs = sample_batched['depth_dirs']

                rgb = Variable(rgb).float().to(device)
                ir = Variable(ir).float().to(device)
                depth = Variable(depth).float().to(device)
                target_var = Variable(target).long().to(device) # binary label

                # compute output
                output = model(rgb, ir, depth) # torch.Size([32, 1024])
                loss = criterion(output, target_var)
                ...
                soft_output = torch.softmax(output,dim=-1) # torch.Size([32, 1024])
                preds = soft_output.to('cpu').detach().numpy() # (32, 1024)
                label = target.to('cpu').detach().numpy() # (32,)
                ...
                for i_batch in range(preds.shape[0]):
                    ...
                    if args.val_save:
                        os.makedirs(f"work_dirs/{args.arch}/submission", exist_ok=True)
                        f = open(f"work_dirs/{args.arch}/submission/{time_stp}_{args.arch}_{epoch}_submission.txt", 'a+') 
    ...
    #     pickle.dump(xy_dic, open('xys/xy_{}_{}_{}.pickle'.format(time_stp, args.arch,epoch),'wb'))
        with open(f"work_dirs/{args.arch}/val_result_{time_stp}_{args.arch}.txt",'a+') as f_result:
            ...
            writer.add_scalar('Epoch/APCER', apcer, epoch)
            writer.add_scalar('Epoch/BPCER', npcer, epoch)
            writer.add_scalar('Epoch/ACER', acer, epoch)
            writer.add_scalar('Epoch/ACC', top1.avg, epoch)
            writer.add_scalar('Epoch/TPR@FPR=10E-2', tprs["TPR@FPR=10E-2"], epoch)
            writer.add_scalar('Epoch/TPR@FPR=10E-3', tprs["TPR@FPR=10E-3"], epoch)
